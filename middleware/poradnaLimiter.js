// Ochrana proti spamu dotazů z poradny velikostí - stejný vzor jako middleware/objednavkyLimiter.js,
// jen s vlastním počítadlem (jiná IP historie by se jinak sdílela mezi formuláři).
...
function zaznamenatDotaz(ip) {
  ...
  data.pocet++;
  if (data.pocet >= MAX_DOTAZU) {
    data.blokovanoDo = ted + BLOKACE_MS;
  }
  pokusy.set(ip, data);
}

module.exports = { jeZablokovana, zaznamenatDotaz };
