// Ochrana proti spamu rezervací - stejný vzor jako middleware/objednavkyLimiter.js,
// jen s vlastním počítadlem (jiná IP historie by se jinak sdílela mezi rezervacemi
// a objednávkami a mohla by zbytečně blokovat legitimní zákazníky).

const MAX_REZERVACI = 5;
...
function zaznamenatRezervaci(ip) {
  const ted = Date.now();
  ...
  data.pocet++;
  if (data.pocet >= MAX_REZERVACI) {
    data.blokovanoDo = ted + BLOKACE_MS;
  }
  pokusy.set(ip, data);
}

module.exports = { jeZablokovana, zaznamenatRezervaci };
